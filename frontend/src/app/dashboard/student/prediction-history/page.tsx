...
export default function PredictionHistoryPage() {
    ...
    const fetchPredictions = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/student/predictions?${queryParams}`, {
            ...
            if (page === 1) { // Only fetch all on first load or filter change
                const allRes = await fetch(`http://localhost:5000/api/student/predictions?limit=1000`, {
                    headers: { "Authorization": `Bearer ${token}` }
                });
